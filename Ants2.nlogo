...
to setup-patches
  repeat number-of-sources [
    let rand-x ((random-float 2) - 1) * max-pxcor * (max-distance-to-nest / 100)
    let rand-y ((random-float 2) - 1) * max-pycor * (max-distance-to-nest / 100)
    ask patches [
...
to setup-nest ;; patch procedure
  set nest? (distancexy 0 0) < 5 ;; set nest? variable to true inside the nest, false elsewhere
  set nest-scent 200 - distancexy 0 0 ;; spread a nest-scent over the whole world -- stronger near the nest
end

to setup-food [x y] ;; patch procedure
  if distancexy (x) (y) < 5 [
    set food one-of [1 2]
  ]
end
...
to recolor-patch ;; patch procedure
  ifelse nest? [ ;; give color to nest and food sources
    set pcolor violet
  ] [
    ifelse food > 0 [
      set pcolor cyan
...
to go  ;; forever button
  ...
  if make-new-ants and random-float 100 < reproduce-chance [create-ant]
  spread-food
  tick
end
...
"" ""
PENS
"food-in-pile" 1.0 0 -11221820 true "" "plotxy ticks sum [food] of patches with [pcolor = cyan]"
